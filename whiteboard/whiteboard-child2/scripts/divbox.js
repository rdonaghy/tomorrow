    ...
    $.fn.divbox = function(opt){
        ...
        function _run(index,init){
            ...
            fn.viewYouTube = function(src,caption){
				...
				// adding &showInfo=0 to the original vidSrc variable to remove title bar from videos
                var vidSrc = "http://www.youtube.com/v/" + vidId + "&hl=en&fs=1&autoplay=1&rel=0&showinfo=0";
                var winW = 640;
                var winH = 385;
                var top = sizesystem[3] + Math.round((sizesystem[5] - winH)/2);
                var left = Math.round((sizesystem[4] - winW)/2);
